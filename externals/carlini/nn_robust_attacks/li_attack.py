...
class CarliniLi:
    ...
    def gradient_descent(self, sess, model):
        ...
        real = tf.compat.v1.reduce_sum((tlab) * output)
        other = tf.compat.v1.reduce_max((1 - tlab) * output - (tlab * 10000))
        ...
        # sum up the losses
        loss2 = tf.compat.v1.reduce_sum(tf.maximum(0.0, tf.abs(newimg - tf.tanh(timg) / 2) - tau))
