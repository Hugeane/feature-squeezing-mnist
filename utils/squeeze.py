...
def reduce_precision_tf(x, npp):
    ...
    """
    npp_int = npp - 1
    x_int = tf.compat.v1.rint(tf.multiply(x, npp_int))
    x_float = tf.compat.v1.div(x_int, npp_int)
    return x_float
...
def bit_depth_random_tf(x, bits, stddev):
    rand_ts = tf.compat.v1.random_normal(x.get_shape(), mean=0, stddev=stddev)
    x_random = tf.compat.v1.add(x, rand_ts)
...
def adaptive_binarize_py(x, block_size=5, C=33.8):
    ...
    import cv2
    ret_imgs = opencv_wrapper(x, cv2.adaptiveThreshold,
...
def non_local_means_color_py(imgs, search_window, block_size, photo_render):
    import cv2
    ret_imgs = opencv_wrapper(imgs, cv2.fastNlMeansDenoisingColored,
...
def non_local_means_color_tf(imgs, search_window, block_size, photo_render):
    ...
    y = tf.numpy_function(my_func, [imgs], tf.float32)
    return y
