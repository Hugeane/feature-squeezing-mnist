...
def keras_resnet50_imagenet_model(logits=False, input_range_type=1):
    ...
    #     x = x + 0.5
    # x_bgr = scaling_tf(x)
    model = ResNet50(include_top=True, weights='imagenet', input_tensor=None, input_shape=input_shape, pooling=None,
                     classes=1000, logits=logits, input_range_type=input_range_type)
...
def keras_vgg19_imagenet_model(logits=False, input_range_type=1):
    ...
    """
    input_shape = (224, 224, 3)
    model = VGG19(include_top=True, weights='imagenet', input_tensor=None, input_shape=input_shape, pooling=None,
                  classes=1000, logits=logits, input_range_type=input_range_type)
    return model
